		...
		$("#fee_collect_frm").validate({
			...
			submitHandler: function(form) {
				...
				$.post('<?php echo base_url(); ?>index.php?admin/fee_collect_process_data/', $("#fee_collect_frm").serialize(), function(data) {
					...
					$('#results').html('<div class="alert alert-success"><button type="button" class="close" data-dismiss="alert">&times;</button><strong>Payment done successfully!</strong><p><button href="<?php echo base_url(); ?>index.php?admin/generatereceipt/'+receipt_id+'" class="btn btn-danger" id="custom-tam-greceipt">Generate receipt</button></p></div>');
					$('#fee_collect_frm')[0].reset();
					$('#process_payment_amount_text').text('0');
					call_data_load();
