	...
	<div class="box-content padded">
    	...
    	<div class="row-fluid">
        		...
        		<div class="span12">
                	    ...
                	    <form class="form-horizontal validatable" method="post" action="<?php base_url() ?>index.php?principal/fee_period_insert">
            	    ...
            	    <table cellpadding="0" cellspacing="0" border="0" class="dTable responsive">
                            ...
                            <?php if(!empty($feeperiods)) { foreach($feeperiods as $feeperiods_view) { ?>
                            	<tr>
                                    ...
                                    <td><a href="<?php echo base_url();?>index.php?principal/fee_period_edit/<?php echo $feeperiods_view->fee_period_id ?>" class="btn btn-small btn-lightblue"><i class="icon-edit"></i> Edit</a>  <a data-toggle="modal" href="#modal-delete" onclick="modal_delete('<?php echo base_url();?>index.php?principal/fee_period_delete/<?php echo $feeperiods_view->fee_period_id ?>')" class="btn btn-small btn-red"><i class="icon-trash"></i> Trash</a></td>
                                </tr>
                            <?php } } ?>    
                            </tbody>
                    </table>
...
$('#fee_period_cid').on('change',function(){
	...
	$('#val_fc_id').append('<img title="<?php echo base_url(); ?>template/images/loading.gif" src="<?php echo base_url(); ?>template/images/loading.gif">');
		...
		$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?principal/fee_particulars_data/',
...
$( "#fee_period_sdate,#fee_period_edate" ).change(function() {
		...
		if(srtDt!='' && enDt!=''){
		 $.ajax({
							type: 'POST',
							url: '<?php echo base_url() ?>index.php?principal/datecompare/',
