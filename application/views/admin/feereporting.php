...
<div class="box">
	<div class="box-content padded">
        ...
        <?php //print_r($student_data);
					 ...
					 $fperiod_data = $ci->fee_model->getfeepperiod($fee_particular_id);
                ...
                <div class="span3">
                	<div class="pinfo">Start Date: <?php //echo date("d-m-Y",strtotime($fee_period_sdate)); ?>
                    <?php
				   $var = $fee_period_sdate;
...
echo date('d-m-Y', strtotime($date));
                ...
                <div class="span3">
                	<div class="pinfo">End Date: <?php //echo date("d-m-Y",strtotime($fee_period_edate)); ?>
                     <?php
				   $var = $fee_period_edate;
$date = str_replace('/', '-', $var);
echo date('d-m-Y', strtotime($date));
				   ?>
                    </div>
                ...
                <div class="span3">
                	<div class="pinfo">Due Date: <?php //echo date("d-m-Y",strtotime($fee_period_ddate)); ?>
                     <?php
				   $var = $fee_period_ddate;
$date = str_replace('/', '-', $var);
//echo date('d-m-Y', strtotime($date));

echo $fee_period_ddate;
				   ?>
                    </div>
                </div>
        ...
        <div class="row-fluid">
        	<div class="span12">
            	<div class="pinfo">
                    ...
                    <button class="btn btn-success" id="custom-notification-unpaid">Send Notification to Unpaid Members</button>
                </div>
            </div>
        </div>
        <?php } ?>
										...
										?>
                                        	...
                                        	<tr>
                                                ...
                                                <td align="right"><?php echo $dueamount; ?>
                                                 ...
                                                 <?php if($dueamount != 0) {  ?> <input type="checkbox" checked="checked" value="<?php echo $student_data_view->student_id ?>" name="smem[]" style="display:block" />  <?php } ?>
...
$('#custom-sms-unpaid').on('click',function(e){
	 ...
	 $('#custom-tam-model-body-sms-unpaid').text('');
			...
			if(val_pass){
				$.ajax({
						type: 'POST',
						url: '<?php echo base_url(); ?>index.php?admin/send_sms_unpaid_mem/',
...
$('#custom-notification-unpaid').on('click',function(e){

	 $('#custom-tam-model-body-notification-unpaid').text('');
			...
			if(val_pass){
				$.ajax({
						type: 'POST',
						url: '<?php echo base_url(); ?>index.php?admin/send_notification_unpaid_mem/',
						data: $( "#unpaid_sms_mul_fm" ).serialize(),
						success: function(data) {
							...
							$( "#custom-tam-model-body-notification-unpaid" ).append( '<p>Notification Send Successfully!</p>');
							}
						}
					});
			}
	         else {
				 $( "#custom-tam-model-body-notification-unpaid" ).append('<p>Unpaid Users Not Found!</p>' );
			 }

	e.preventDefault();
	$('#custom-tam-model-notification-unpaid').modal('show');
