<div class="box">
	...
	<div class="box-content padded">
        ...
        <a href="javascript:void(0);" class="btn btn-warning custom-tam-table-btns" id="custom-tam-smaila">Send Mail to Absenties</a>
...
$('#custom-tam-ssmsa').on('click',function(e){
	...
	else
	{
		...
		$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?admin/daily_attn_count',
							data: { smscid:smsclassid ,smsdt:smsdate },
							success: function(data) {
								
								if(data=='true'){
									$('#custom-tam-mode').text('').text('Send SMS to Absenties');
...
$(document).on('click','#sms-tam-absenties',function(){
	...
	$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?admin/sendsmsabsenties',
							data: { smscid:smsclassid ,smsdt:smsdate },
							success: function(data) {
							//alert(data);
								if(data!=''){
...
$('#custom-tam-smaila').on('click',function(e){
	...
	else
	{
		...
		$.ajax({
							...
							success: function(data) {
								...
								if(data=='true'){
									$('#custom-tam-mode').text('').text('Send Mail to Absenties');
									 $( "#custom-tam-model-body" ).append('<div class="alert alert-info"><strong>Are you sure!</strong> Send Mail to absenties for selected class and date. </div><div><button class="btn btn-danger" id="mail-tam-absenties" value="Send Mail">Send</button></div>');
...
$(document).on('click','#mail-tam-absenties',function(){
	...
	$.ajax({
							type: 'POST',
							url: '<?php echo base_url(); ?>index.php?admin/sendmailabsenties',
							data: { smscid:smsclassid ,smsdt:smsdate },
							success: function(data) {
							//alert(data);
								if(data=='true'){
									
									$( "#custom-tam-model-body" ).text('').append('<div class="alert alert-success"><strong>Mail Send Successfully!</strong></div>');
								}
