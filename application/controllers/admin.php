...
/*
 * 	@author : Varun
 * 	TAM Manager School & College Management System
 * 	http://www.tammanager.com
 */
...
//        $this->dynamic_load->add_js('footer', array('src' => asset_url('js','core/bootstrap.js'), 'type' => 'text/javascript'));
        ...
        if ($day = $this->input->post('day')) {
            ...
            if (!$this->input->post('data')) {
                ...
                $this->calendar_model->add_calendar_data("$year-$month-$day", $this->input->post('data'), $this->input->post('category_id'));
            }
        }
...
';
        ...
        $page_data['calender'] = $this->month_cal->generate($year, $month, $cal_data);

        $this->db->where('eventcategory_status', 1);

        $page_data['event_category'] = $this->db->get_where('event_category')->result_array();
...
<div class="controls"><select id="rec_c_id" name="rec_c_id" class="uniform validate[required]"><option value="">----select class----</option><option value="all">ALL</option>';
    ...
    function sendmailsms() {
	 ...
	 /* ######################## HOME WORK MANAGE MAIL SMS ############################### */

    function homeworkmailsms() {

        if ($this->session->userdata('admin_login') != 1)
            redirect(base_url(), 'refresh');

		$this->load->model('fee_model');
        $page_data['sm_class_data'] = $this->fee_model->getclass();

        $page_data['page_name'] = 'homeworkmailsms';
        $page_data['page_title'] = get_phrase('home_work_mail / sms');
        $this->load->view('index', $page_data);
    }

	function hwsendmailsms(){

		$hwsm_cls = $this->input->post('sm_class_chk');
		foreach($hwsm_cls as $hwsm_cls_view) {
		$hwsm_val.= $hwsm_cls_view.',';
		}
	    $hwsm_cls_view = rtrim($hwsm_val, ',');
		$sids = explode(",",$hwsm_cls_view);
		$this->db->where_in('class_id', $sids);
		$sm_qry = $this->db->get('student')->result();
		...
		if ($rmthd == 1) {
            ...
            $sus_sm = send_tam_sms($smsto, $msg);
             ...
             if($sus_sm) $this->session->set_flashdata('msg', 'SMS Send Success');
            redirect(base_url() . 'index.php?admin/homeworkmailsms/', 'refresh');
        } if ($rmthd == 2) {
			$email_subject = $this->input->post('smmsgt');
            $email_body = $this->input->post('smmsg');
			 ...
			 foreach ($sm_qry as $sm_qry_dta) {
                    ...
                    if (!empty($sm_qry_dta->parent_email)) {
                        $parent_email[] = $sm_qry_dta->parent_email;
                        $parent_id[] = $sm_qry_dta->student_id;
                    }
                }
                if (!empty($parent_id)) {
                    $parent_read = implode(',', $parent_id);
    ...
    function student($param1 = '', $param2 = '', $param3 = '') {
        ...
        if ($param1 == 'create') {
            ...
            $data['transport_id'] = $this->input->post('student_transport_id');
			$data['ppoint_id'] = $this->input->post('student_pickup_id');
            ...
            $data['birthday'] = $this->input->post('birth_year') . '-' . $this->input->post('birth_month') . '-' . $this->input->post('birth_day');
          ...
          /*  $system_name = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;
           ...
           // $res = send_tam_sms($to, $msg);
            $res=$this->send_mail("Student Registration", $data['email'],"Bits@demo.com",$msg);


           // $res = send_tam_sms($to, $msg);
            $res=$this->send_mail("Student Registration", "abhik214@gmail.com","Bits@demo.com",$msg);*/
        ...
        $page_data['class_id'] = $param1;
		$this->db->select('class.name cname,name_numeric,student.*,transport.route_name, transport_points.ppoint');
		$this->db->join('transport', 'transport.transport_id = student.transport_id', 'left');
		$this->db->join('transport_points', 'transport_points.pid = student.ppoint_id', 'left');
		$this->db->join('class','student.class_id = class.class_id');

        $page_data['students'] = $this->db->get_where('student', array(
                    'student.class_id' => $param1
                ))->result_array();
...
//  Loop through each row of the worksheet in turn
                    ...
                    for ($row = 2, $i = 0; $row <= $highestRow; $row++) {
                        ...
                        $admission_number = $rowData[$i][0];
                        $roll_no = $rowData[$i][1];
                        $name = $rowData[$i][2];
                        $dob = $rowData[$i][3];
                        $birthplace = $rowData[$i][4];
                        $gender = $rowData[$i][5];

                        $religion = $rowData[$i][6];
                        $cast = $rowData[$i][7];
                        $blood_group = $rowData[$i][8];
                        $nationality = $rowData[$i][9];

                        $present_address = $rowData[$i][10];
                        $permanent_address = $rowData[$i][11];

                        $father_name = $rowData[$i][12];
                        $mother_name = $rowData[$i][13];

                        $parent_phone1 = $rowData[$i][14];
                        $parent_phone2 = $rowData[$i][15];

                        $parent_email = $rowData[$i][16];
                        $parent_username = $rowData[$i][17];
                        $parent_password = $rowData[$i][18];

                        $height = $rowData[$i][19];
                        $weight = $rowData[$i][20];

                        $passport_number = $rowData[$i][21];
                        $date_of_issue = $rowData[$i][22];
                        $date_of_expiry = $rowData[$i][23];

                        $student_qatar_id = $rowData[$i][24];
                        $father_qatar_id = $rowData[$i][25];
                        $mother_qatar_id = $rowData[$i][26];

						$transport_id = $rowData[$i][27];
                        $ppoint_id = $rowData[$i][28];
...
// print_r($rowData);
     ...
     address,permanent_address,previous_school_name,class_id, admission_number, height,weight,passport_number, date_of_issue,date_of_expiry,student_qatar_id, father_qatar_id,mother_qatar_id , transport_id,ppoint_id) values('$roll_no','$name',
        '$gender','$dob','$father_name','$mother_name','$birthplace','$blood_group',
         ...
         '$income','$present_address','$permanent_address','$previous_school','$class','$admission_number', '$height','$weight','$passport_number', '$date_of_issue','$date_of_expiry','$student_qatar_id', '$father_qatar_id','$mother_qatar_id','$transport_id','$ppoint_id')");

                                                        $details['item'][] = "<div style='color:#1171A3;'>The Roll No $roll_no inserted successfully... </div>";

														$this->email_model->account_opening_email_parent($parent_email);
                                                    } else {
...
function teacher($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'do_update') {
        ...
        redirect(base_url() . 'index.php?admin/teacher/'.$this->input->post('employee_department_id'), 'refresh');
    ...
    if ($param1 == 'do_upload') {
        ...
        if ($_POST['isupload']) {
            $department_id = $_POST['u_department_id'];
            //$values=$this->validate($_POST);
...
//  Loop through each row of the worksheet in turn
                ...
                for ($row = 2, $i = 0; $row <= $highestRow; $row++) {
                    ...
                    $edata['employee_department_id'] = $department_id;

                    $edata['name'] = $rowData[$i][0];
                    $edata['employee_code'] = $rowData[$i][1];
                    $edata['birthday'] = $rowData[$i][2];
                    $edata['sex'] = $rowData[$i][3];
                    $edata['religion'] = $rowData[$i][4];
                    $edata['cast'] = $rowData[$i][5];
                    $edata['blood_group'] = $rowData[$i][6];
                    $edata['address'] = $rowData[$i][7];
                    $edata['permanent_address'] = $rowData[$i][8];
                    $edata['subject'] = $rowData[$i][9];
                    $edata['father_name'] = $rowData[$i][10];
                    $edata['father_mobile_number'] = $rowData[$i][11];
                    $edata['teacher_email'] = $rowData[$i][12];
                    $edata['phone'] = $rowData[$i][13];
                    $edata['email'] = $rowData[$i][14];
                    $edata['password'] = $rowData[$i][15];



                    if (isset($edata['employee_department_id']) && $edata['employee_department_id'] != '') {
                        $this->db->insert('teacher', $edata);
                    } //first if
            ...
            redirect(base_url() . 'index.php?admin/teacher/' . $department_id . '');
        }
    ...
    if ($param1 == 'delete') {
        $arr = $this->db->where('teacher_id', $param2)->get('teacher')->result_array();
        $department_id = $arr[0]['employee_department_id'];
        $this->db->where('teacher_id', $param2);
        $this->db->delete('teacher');
        redirect(base_url() . 'index.php?admin/teacher/' . $department_id, 'refresh');
    }
...
function frontdeskenquiry($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($edate != '') {
        ...
        $page_data['edate'] = $date;



        $edt = date('Y-m-d', strtotime($date));
...
function taskinsert() {
    ...
    //$date1 = str_replace('/', '-', $this->input->post('task_start_date'));
    $date1 = $this->input->post('task_start_date');

    //$date2 = str_replace('/', '-', $this->input->post('task_end_date'));
    $date2 = $this->input->post('task_end_date');

    $taskdata['task_title'] = $this->input->post('task_title');
    $taskdata['task_description'] = $this->input->post('task_description');
    $taskdata['task_start_date'] = date("Y-m-d", strtotime($date1));
    $taskdata['task_end_date'] = date("Y-m-d", strtotime($date2));
...
function homework($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'create') {
		...
		$hw_cls_val = "";
        foreach ($this->input->post("hw_class_chk") as $hw_cls_id) {
            $hw_cls_val .="," . $hw_cls_id . ",";
        }

	    $data['hw_cid'] = $hw_cls_val;
        $data['hw_title'] = $this->input->post('hw_title');
        $data['hw_data'] = $this->input->post('hw_data');

		$data['hw_added_date'] =  date('Y-m-d H:i:s');

		$data['hw_modified_date'] =  date('Y-m-d H:i:s');

        $date = $this->input->post('create_timestamp');
        // $date = str_replace('/', '-', $date);

        $data['hw_date'] = date('Y-m-d',strtotime($date));


        $this->db->insert('homework', $data);
        redirect(base_url() . 'index.php?admin/homework/', 'refresh');
    }
    if ($param1 == 'do_update') {
        ...
        $this->db->where('hw_id', $param2);
        $this->db->update('homework', $data);
        $this->session->set_flashdata('flash_message', get_phrase('home_work_updated'));
        redirect(base_url() . 'index.php?admin/homework/', 'refresh');
    } else if ($param1 == 'edit') {
        $page_data['edit_data'] = $this->db->get_where('homework', array(
                    'hw_id' => $param2
                ))->result_array();
    }
    if ($param1 == 'delete') {
	...
	$this->load->model('fee_model');
    $page_data['hw_class_data'] = $this->fee_model->getclass();

    $page_data['page_name'] = 'homework';
    $page_data['page_title'] = get_phrase('manage_homework');
	$this->db->where('hw_delete', 0);
    $page_data['homeworks'] = $this->db->get('homework')->result_array();
    $this->load->view('index', $page_data);
}
...
function classnoticeboard($param1 = '', $param2 = '', $param3 = '') {
    ...
    if ($param1 == 'do_update') {
        ...
        $this->session->set_flashdata('flash_message', get_phrase('notice_updated'));
        redirect(base_url() . 'index.php?admin/classnoticeboard/', 'refresh');
    } if ($param1 == 'staff_notice_do_update') {
        $data['notice_department'] = $this->input->post('notice_department');
        $data['notice_title'] = $this->input->post('notice_title');
        $data['notice'] = $this->input->post('notice');
        $date = $this->input->post('create_timestamp');
        //// $date = str_replace('/', '-', $date);
        ...
        $data['create_timestamp'] = strtotime($date);
        $this->db->where('notice_id', $param2);
        $this->db->update('staffnotice', $data);
        $this->session->set_flashdata('flash_message', get_phrase('notice_updated'));
        redirect(base_url() . 'index.php?admin/classnoticeboard/', 'refresh');
    } else if ($param1 == 'edit') {
    ...
    if ($param1 == 'staff_notice_delete') {
        $this->db->where('notice_id', $param2);
        $this->db->delete('staffnotice');
        redirect(base_url() . 'index.php?admin/classnoticeboard/', 'refresh');
    }
    if ($param1 == 'create_staff_notice') {
        $data['notice_department'] = $this->input->post('notice_department');
        $data['notice_title'] = $this->input->post('notice_title');
        $data['notice'] = $this->input->post('notice');
        ...
        $data['create_timestamp'] = strtotime($date);
        $this->db->insert('staffnotice', $data);
        redirect(base_url() . 'index.php?admin/classnoticeboard/', 'refresh');
    }

    $page_data['page_name'] = 'classnoticeboard';
    $page_data['page_title'] = get_phrase('class_/_Staff_noticeboard');
    ...
    $this->db->join('departments', 'notice_department = department_id');
    $page_data['staffnotices'] = $this->db->get('staffnotice')->result_array();
...
function daily_attn_count() {
	...
	$date = str_replace('/', '-', $this->input->post('smsdt'));
    $smsdate = date("Y-m-d", strtotime($date));
...
function daily_staffattn_count() {
    if ($this->session->userdata('admin_login') != 1)
        redirect(base_url(), 'refresh');

    $smsstaffid = $this->input->post('smscid');
	$date = str_replace('/', '-', $this->input->post('smsdt'));
    $smsdate = date("Y-m-d", strtotime($this->input->post('smsdt')));

    $this->db->where('present_date', $smsdate);
    $this->db->where('present_flag', 'A');
    $this->db->where('staff_type', $smsstaffid);
...
function sendsmsabsenties() {
    ...
    foreach ($smsdata as $smsdata_view) {
        ...
        echo $res = send_tam_sms($to, $msg);
    }
}

function sendmailabsenties() {
    ...
    $this->db->select('student.name as sname,student_parent_email as pemail,father_name as fname');
    $this->db->from('daily_attendence,student');
    $this->db->where('daily_attendence.present_date', $smsdate);
    $this->db->where('daily_attendence.present_flag', 'A');
    $this->db->where('daily_attendence.class_id', $smsclassid);
    ...
    foreach ($smsdata as $smsdata_view) {
        ...
        $tomail = $smsdata_view->pemail;

        $msg = "Your children " . $sid . " absent school today (" . $smsdate . ") please inform the reason";

		$data['fathername'] = $smsdata_view->fname;

		$data['message'] = $msg;

		$data['mdate'] = $smsdate;

		$mesg = $this->load->view('absentiesmail', $data, true);

        $system_email = $this->db->get_where('settings', array('type' => 'system_email'))->row()->description;

        $system_name = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;

        $res = $this->email_model->do_email($mesg, $system_name . " - Absent Notification ".$smsdate, $tomail, $system_email);
...
function sendsmsstaffabsenties() {
    ...
    $this->db->select('teacher.phone');
    $this->db->from('daily_staffattendence,teacher');
    $this->db->where('daily_staffattendence.present_date', $smsdate);
    $this->db->where('daily_staffattendence.present_flag', 'A');
    $this->db->where('daily_staffattendence.staff_type', $smsclassid);
    $this->db->where('teacher.teacher_id = daily_staffattendence.staff_id');
    $query = $this->db->get_where();
    $smsdata = $query->result();

    foreach ($smsdata as $smsdata_view) {
        //$sid = $smsdata_view->name;

        $to = $smsdata_view->phone;

        $msg = "You absent on school today (" . $smsdate . ")";
    ...
    if ($param1 == 'create') {
        ...
        foreach ($_FILES as $key => $val) {
            ...
            foreach ($datas['book_file'] as $key => $image) {
                ...
                $data['book_file'] = time() . "_" . $name;
                $path = "uploads/ebook_image/" .$data['book_file'];
                //print_r($data);
...
function departmentcheck() {
    ...
    $department = trim($this->input->post('chkdepartment'));

    $this->db->where('department_name', $department);

    $department_chk_count = $this->db->count_all_results('departments');

    if ($department_chk_count > 0) {
...
/*    Check examination name exists or not      */

function examinationcheck() {



    $exam = trim($this->input->post('chkexam'));

    $this->db->where('name', $exam);

    $exam_chk_count = $this->db->count_all_results('exam');
...
function fee_cat_delete($fc_id) {
    ...
    if ($fc_id != '') {
        ...
        $this->fee_model->update_cfeeparticular($fpdata, $fc_id);
...
function fee_collect_process_data() {
    ...
    if ($res) {
        ...
        $this->db->select('name as sname,parent_phone1 as pphone,student_parent_email');
        ...
        $data['receipt_id'] = $rndm_str;

        $mesg = $this->load->view('receiptmail', $data, true);

        $system_email = $this->db->get_where('settings', array('type' => 'system_email'))->row()->description;

        $system_name = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;

        $this->email_model->do_email($mesg, $system_name . " - Payment Receipt on -" . $fee_collect_data['fee_collection_added_date'], $student_parent_email, $system_email);
...
function getreceiptmail() {


    $data['receipt_id'] = 'ifHk-7318-mkcBZ-3864-DGXq';

    echo $mesg = $this->load->view('receiptmail', $data, true);
}
...
function send_sms_unpaid_mem() {
    ...
    foreach ($data1 as $key => $value) {
        ...
        $this->db->select('name as sname,parent_phone1 as pphone,student_parent_email as pemail,father_name as fname');
        ...
        $mesg = $this->load->view('duemail', $data, true);

        $system_email = $this->db->get_where('settings', array('type' => 'system_email'))->row()->description;

        $system_name = $this->db->get_where('settings', array('type' => 'system_name'))->row()->description;

        $this->email_model->do_email($mesg, $system_name . " - Due Notification", $pemail, $system_email);
...
function send_notification_unpaid_mem() {
	...
	if (!empty($parent_id)) {
				...
				$msg = "Please payment amount for " . $pname . "  on or before due date " . $pddate ;

				$email_subject = $pname." due notification ";
                $data = array();
                $data['unread'] = $parent_read;
                $data['read'] = "";
                $data['time'] = date("Y-m-d H:i:s");
...
function eventcategory($param1 = "", $param2 = "", $param3 = "") {
    ...
    if ($param1 == 'create') {
        ...
        $data['eventcategory_name'] = $this->input->post('name');
        $data['eventcategory_bg'] = $this->input->post('ec_bg');
        $data['eventcategory_fc'] = $this->input->post('ec_fc');
        $data['eventcategory_status'] = $this->input->post('status');
        $data['eventcategory_added_date'] = date('Y-m-d H:i:s');
        $data['eventcategory_modified_date'] = date('Y-m-d H:i:s');

        $this->db->insert('event_category', $data);

        redirect(base_url() . 'index.php?admin/eventcategory/', 'refresh');
    } else if ($param1 == 'edit') {
        $page_data['edit_data'] = $this->db->get_where('event_category', array(
                    'eventcategory_id' => $param2
                ))->result_array();
        ...
        $this->db->where('eventcategory_id', $param2);
    ...
    $page_data['page_name'] = 'eventcategory';

    $page_data['page_title'] = get_phrase("event_/_calendar_category");
...
';
        ...
        $data['event_category'] = $this->db->get_where('event_category')->result_array();

        $deta['page_title'] = $this->lang->line("calendar");
...
function sms_student_snd() {
   ...
   echo $res = send_tam_sms($sphone, $smsg);
exit;
    if ($res) {
        echo "true";
    } else {
...
/* * *******     Teacher Send SMS and Email functions      ********* */

function sms_teacher_data() {
    $sid = $this->input->post('sid');
    ...
    echo '<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			<h3 id="custom-tam-mode">Send SMS to ' . $name . ' (Teacher Name)</h3>
			</div>
			<div class="modal-body">
			<div id="sms-overlay"><img src="' . base_url() . '/template/images/ajax-loader-2.gif"></div>
			<div id="sms-feedback"></div>
			    <form class="form-horizontal" method="post" action="#" id="snd-frm">
				<div class="control-group">
				<label class="control-label" for="inputEmail">Teacher Name</label>
				<div class="controls">
				<p class="cstm-sts-txt">' . $name . '</p>
				</div>
				</div>
				<div class="control-group">
				<label class="control-label" for="inputEmail">Teacher Phone</label>
				<div class="controls">
				<p class="cstm-sts-txt">' . $phone . '</p>
				</div>
				</div>
				<div class="control-group">
				<label class="control-label" for="inputPassword">Message</label>
				...
				<div class="control-group">
				<div class="controls">
				<input type="hidden" value="' . $phone . '" id="smsphone" name="smsphone">
				<button type="button" class="btn btn-success" id="sndsmsbtn">Send SMS</button>
				</div>
				</div>
				</form>
...
function email_teacher_data() {
    ...
    echo '<div class="modal-header">
			<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
			<h3 id="custom-tam-mode">Send Email to ' . $name . ' (Teacher Name)</h3>
			</div>
			<div class="modal-body">
			<div id="sms-overlay"><img src="' . base_url() . '/template/images/ajax-loader-2.gif"></div>
			<div id="sms-feedback"></div>
...
function gallery($param1 = '', $param2 = '') {
    ...
    $page_data['records'] = $this->db->query("select * from  tbl_gallery_category where category_status=1")->result_array();
    $page_data['page_name'] = 'Category';
    if ($this->input->post()) {
        $category = $this->input->post('category');
        $records = $this->db->query("select * from  tbl_gallery_category where category_name='$category'")->result_array();
        if (count($records) == 0) {
    ...
    $this->load->view('admin/gallery', $page_data);
}

function gallerys($param1 = '', $param2 = '') {
    if ($this->session->userdata('admin_login') != 1)
        redirect('login', 'refresh');

    $page_data['records'] = $this->db->query("select * from  tbl_gallery_category where category_status=1")->result_array();
    foreach ($page_data['records'] as $names) {
        $page_data['categorys'][$names['category_id']] = $names['category_name'];
    }
    if ($param1 == 'delete') {
        $this->db->where('screen_id', $param2);
        $this->db->delete('tbl_gallery');
        redirect(base_url() . 'index.php?admin/gallerys', 'refresh');
    }
    $galleryQuery = "select * from  tbl_gallery where screen_cat!=0";
    if ($param1) {
        $param1 = addslashes($param1);
        $page_data['cat_id'] = $param1;
        $galleryQuery .= " AND screen_cat = '" . $param1 . "'";
    }

    $page_data['records_gall'] = $this->db->query($galleryQuery)->result_array();
    //print_r($page_data['records_gall']); exit;
    $page_data['page_name'] = 'Gallery Management';
    //print_r($_FILES);
    if (!empty($_FILES['file']['tmp_name'])) {
        if ($_FILES['file']['type'] == 'image/png' || $_FILES['file']['type'] == 'image/jpeg' || $_FILES['file']['type'] == 'image/jpg' || $_FILES['file']['type'] == 'image/gif') {
            $image_paths = "uploadimages/" . "image_" . rand(1, 999999) . "_" . $_FILES['file']['name'];
            var_dump(copy($_FILES['file']['tmp_name'], $image_paths));
            if (copy($_FILES['file']['tmp_name'], $image_paths)) {
                $data['screen_image'] = $image_paths;
                $data['screen_cat'] = $_POST['category'];
                $this->db->insert('tbl_gallery', $data);
                //redirect(base_url() . "index.php?admin/gallerys");
                echo 'Success'; exit;
            } else {
                //redirect(base_url() . "index.php?admin/gallerys");
                echo 'Failed'; exit;
    ...
    $page_data['page_title'] = get_phrase('Gallery Management');
    $this->load->view('admin/gallerys', $page_data);
}

function deleteGalleryImage() {
    $galleryId = $_POST['galleryId'];

    $this->db->where('screen_id', $galleryId);
    $this->db->delete('tbl_gallery');
}
...
public function cce_class_indicator1($param1 = '') {
...
function cce_resultcommentsentry($param1 = '', $param2 = '') {
...
function cce_resultcomments_ins() {
    $cr_id_data = $this->input->post('cr_id');

    foreach ($cr_id_data as $key => $cr_id) {

        $cr_remarks = $this->input->post("remarks");
        $data['cr_remarks'] = $cr_remarks[$key];

        $cr_comments = $this->input->post("comments");
        $data['cr_comments'] = $cr_comments[$key];

        $this->db->where('cr_id', $cr_id);
        $this->db->update('cce_resultcomments', $data);
    }

    $this->session->set_flashdata('flash_message', get_phrase('CCE_Scholastic_Comments_Entry_updated_successfully !'));
    redirect(base_url() . 'index.php?admin/cce_resultcommentsentry/' . $this->input->post('hid_cid') . '/' . $this->input->post('hid_tid'), 'refresh');
}

function send_mail($subject,$to,$from,$msg)
                        {
                              ...
                              'smtp_port' => 465,
                              'smtp_user' => 'bitstestdemo@gmail.com', // change it to yours
                              'smtp_pass' => 'qwertykeyboard', // change it to yours
                              'mailtype' => 'html',
                              'charset' => 'iso-8859-1',
                              'wordwrap' => TRUE
                              );
...
function customsearch($param1 = ''){

	if($param1 == 'search'){

		$src_type = $this->input->post('search_type');
		$src_data = $this->input->post('search_data');
		if($src_type == 'Admission_Number'){

			$this->db->select('student.* , class.name as cname , name_numeric,transport.route_name, transport_points.ppoint ');
			$this->db->join('class','class.class_id = student.class_id');
			$this->db->join('transport', 'transport.transport_id = student.transport_id', 'left');
		    $this->db->join('transport_points', 'transport_points.pid = student.ppoint_id', 'left');
			$this->db->where('admission_number', $src_data);

			$result = $this->db->get('student');

			$page_data['student_data'] = $result->result_array();

			$page_data['search_data'] = $src_data;

			$page_data['search_type'] = $src_type;

		} else if($src_type == 'Student_Name') {

			$this->db->select('student.* , class.name as cname , name_numeric,transport.route_name, transport_points.ppoint ');
			$this->db->join('class','class.class_id = student.class_id');
			$this->db->join('transport', 'transport.transport_id = student.transport_id', 'left');
		    $this->db->join('transport_points', 'transport_points.pid = student.ppoint_id', 'left');
			$this->db->like('student.name', $src_data, 'after');
	...
	$page_data['page_name'] = 'customsearch';
